/* MINOR INDEX — generated by mkbundle.js. Do not edit: edit the modules.
   _mark.js + _glyphs.js + _letters.js + _font.js + _studio.js → one file, each module still in its own scope. */
(function (global) {
"use strict";
...
function nameTable(records) {
  const items = records.map(([id, str]) => ({
    id,
    mac: Buffer.from(str, "latin1"),
    win: Buffer.from(str.split("").map((c) => " " + c).join(""), "latin1"),
  }));
  const heads = [], datas = [];
  let off = 0;
